...
const Card = ({ cardData, onDeleteCard }: CardProps) => {  
  const { attributes, listeners, setNodeRef, transform, transition, isDragging } = useSortable({ id: cardData._id });
  const style = {
  ...
  return (
    <div
      ...
      {...listeners}
      style={style}
      className={`divide-y divide-gray-200 overflow-hidden rounded-lg shadow-sm ${isDragging ? 'bg-gray-200' : 'bg-white'}`}
    >
